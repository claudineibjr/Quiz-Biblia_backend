import Question, {QuestionDificulty, QuestionFilter} from "../Model/Question";

import QuestionDB from './Firebase/CloudFirestore/QuestionDB';
...
export default class QuestionServices {
    ...
    public static getQuestion(userAnswered: Array<Number>, questionFilter: QuestionFilter): Promise<Question> {
        return new Promise((resolve, reject) => {
            resolve(QuestionDB.getQuestion(userAnswered, questionFilter));
        });
    }

    public static getQuestionById(idQuestion: number): Promise<Question> {
        return new Promise((resolve, reject) => {
            resolve(QuestionDB.getQuestionById(idQuestion));
        });
    }
}